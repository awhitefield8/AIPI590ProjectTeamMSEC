...
def parse_args():
    ...
    parser.add_argument('--data', nargs='?', default='data_rr',
                        help='data directory')
        ...
        with tf.compat.v1.variable_scope(self.name):
            ...
            if self.model=='GRU':
                gru_out, self.states_hidden = tf.compat.v1.nn.dynamic_rnn(
                    tf.compat.v1.nn.rnn_cell.GRUCell (self.hidden_size), # tf.compat.v1.estimator.rnn.GRUCell(self.hidden_size)
                    self.input_emb,
                    dtype=tf.float32,
            ...
            self.loss1 = tf.reduce_mean(input_tensor=ce_loss_pre)
            self.opt1 = tf.compat.v1.train.AdamOptimizer(learning_rate).minimize(self.loss1)

            self.loss2 = tf.reduce_mean(input_tensor=ce_loss_post)
...
def evaluate(sess):
    ...
    for i in range(len(topk)):
        ...
        dictionary[f"cumulative reward top {i}"] = total_reward[i]
        dictionary[f"clicks hr top {i}"] = hr_click
        dictionary[f"clicks ndcg top {i}"] = ng_click
        dictionary[f"purchase hr top {i}"] = hr_purchase
        dictionary[f"purchase ndgc top {i}"] = ng_purchase
